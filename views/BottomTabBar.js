...
import { Animated, TouchableWithoutFeedback, StyleSheet, View, Platform, Dimensions } from 'react-native'
...
class TabBarBottom extends React.Component {
  ...
  animaToIndex = (index) => {
    this._waveRect.startAnim(index)
  }
      ...
      <View>
        ...
        <View style={tabBarStyle}>
          {routes.map((route, index) => {
            ...
            return <ButtonComponent key={route.key} onPress={() => {
              this.animaToIndex(index)
              onTabPress({ route })
            }} testID={testID} accessibilityLabel={accessibilityLabel} style={[styles.tab, { backgroundColor }, this._shouldUseHorizontalLabels() ? styles.tabLandscape : styles.tabPortrait, tabStyle]}>
              {this._renderIcon(scene)}
              {this._renderLabel(scene)}
            </ButtonComponent>
          })}
        </View>
        <SafeAreaView style={{ backgroundColor: '#F7F7F7' }} forceInset={safeAreaInset}/>
      </View>
    )
  }
}
